...
Проект представляет собой ***полностью автоматизированную*** систему обработки данных, разворачиваемую одной командой. Конфигурация проекта максимально облегчена для запуска на локальной машине.
...
С помощью Apache Airflow настроена репликация данных из PostgreSQL в MySQL. DAG выполняет следующие задачи:
1. Извлечение данных из PostgreSQL.
2. Трансформация данных с использованием Spark.
...
#### Описание
Витрина для последующего анализа поведения пользователей, включая количество заказов и общую сумму затрат, разбитых по статусам заказов. 
...
### Витрина продаж продуктов

#### Описание
Витрина для последующего анализа продаж товаров с учетом количества проданных единиц и общей выручки, разбитых по статусам заказов. Обеспечивает понимание популярности товаров и их влияния на общий доход. Также позволяет анализировать эффективность категорий товаров и их соответствие ожиданиям клиентов.
...
2. Выполнение трансформаций, включая объединения, группировки и агрегации.
...
3. Запустить команду:
    ...
    ```
4. После сборки проекта (10-15 минут) и его развертывания (5-10 минут) будут доступны интерфейсы PostgreSQL, MySQL, Airflow и Spark по указанным выше URL.
